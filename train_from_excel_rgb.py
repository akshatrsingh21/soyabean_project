    ...
    def analyze_variety_patterns(self):
        ...
        for variety, color in colors.items():
            ...
            plt.scatter(variety_data['Average R'], variety_data['Average G'], 
                       alpha=0.6, label=variety, color=color)
        ...
        for variety, color in colors.items():
            variety_data = self.data[self.data['Variety'] == variety]
            plt.hist(variety_data['Brightness'], alpha=0.6, label=variety, color=color, bins=20)
        ...
        for variety, color in colors.items():
            variety_data = self.data[self.data['Variety'] == variety]
            plt.hist(variety_data['R_G_Ratio'], alpha=0.6, label=variety, color=color, bins=20)
        ...
        plt.tight_layout()
        plt.savefig('variety_patterns_analysis.png')
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python train_from_excel_rgb.py <path_to_excel_file>")
        return
    
    excel_path = sys.argv[1]
    
    # Initialize trainer
    trainer = ExcelBasedTrainer(excel_path)
